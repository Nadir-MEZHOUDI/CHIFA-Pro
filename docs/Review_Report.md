...
The codebase has a solid high-level direction (WinForms client + service layer), and it builds cleanly on the current stack. The main risks are concentrated in runtime safety (server lifecycle), security/configuration hygiene, and maintainability drift caused by mixed responsibilities and duplicated patterns across projects.
...
1. **Deadlock risk in service server startup/stop path**  
   - `Service lifecycle:19` acquires `_serverLock` in `StartAsync`, then calls `Stop()` at `Service lifecycle:23`, and `Stop()` tries to acquire the same lock again at `Service lifecycle:53`. This can block startup permanently.

2. **Unsafe disposal sequence in service server**  
   - `Service lifecycle:92` disposes `_serverLock` before calling `Stop()` at `Service lifecycle:93`. This can trigger disposal/cleanup races and unobserved failures.

3. **Secrets and credentials embedded in source**  
   - DB password defaults exist in `CHIFA Pro/Helpers/DbChecker.cs:19`, `CHIFA Pro/Helpers/Settings/AppSettings.cs:32`, and `Application/Helpers/Settings/AppSettings.cs:33`.  
   - Bridge API codes are hardcoded in `Application/Views/MainWindow.xaml.cs:30` and `Application/Views/MainWindow.xaml.cs:32`.
...
1. **Shared mutable state via singleton services + mutable `Period`**  
   - `CHIFA.Services/DataServices/ChifaService.cs:10`, `CHIFA.Services/DataServices/StatisticsService.cs:11`, `CHIFA.Services/Dtos/Period.cs:7`.  
...
2. **In-memory post-processing on potentially large datasets**  
   - Methods load large lists then group/filter in memory, e.g. `CHIFA.Services/DataServices/ChifaService.cs:237`, `CHIFA.Services/DataServices/ChifaService.cs:399`, `CHIFA.Services/DataServices/StatisticsService.cs:218`.  
   This increases memory pressure and latency as data grows.

3. **Resource lifetime inconsistencies**  
   - Missing `await using` in `CHIFA.Services/DataServices/ChifaService.cs:344` and non-disposed DB context in `CHIFA Pro/Views/frmMain.cs:272`.

4. **Busy-loop background logging updater**  
   - `Application/Views/MainWindow.xaml.cs:47` runs `while (true)` with UI-dispatch every 500ms (`Application/Views/MainWindow.xaml.cs:53`, `Application/Views/MainWindow.xaml.cs:56`) and no cancellation strategy.
...
6. **Service contract couples to persistence models**  
   - `CHIFA.Services/DataServices/ChifaService.cs:10` and related signatures expose `DataModel` entities/expressions at contract boundary. This makes future schema evolution harder.
...
2. **Duplicate infrastructure helpers across projects**  
   - Similar `AppStartup`, `SettingsToRegistry`, and exception-log helpers are duplicated between client and server (`CHIFA Pro/Helpers/AppStartup.cs:5`, `Application/Helpers/AppStartup.cs:6`, `CHIFA Pro/Helpers/Settings/SettingsToRegistry.cs:6`, `Application/Helpers/Settings/SettingsToRegistry.cs:8`).
...
4. **Magic numbers still present outside threshold constants**  
   - Example: `CHIFA.Services/Helpers/MedicalThresholds.cs` exists, but raw values remain in places like `CHIFA.Services/Dtos/TraitDetailsDto.cs:13` and timing/retry values in `Service timing values` and `Service lifecycle:14`.
...
1. **Fix server lifecycle first (no architectural rewrite)**  
   ...
   - Implement `IAsyncDisposable` for `Service` and await shutdown deterministically.
...
1. **Current project split (`CHIFA Pro`, `Application`, `CHIFA.Services`, `CHIFA.Services`)**: the separation is directionally correct.
...
5. **Medical threshold centralization concept** in `CHIFA.Services/Helpers/MedicalThresholds.cs:3` should be kept and expanded.

## 7. Priority order

1. **Immediate (P0)**: fix `Service` lock/dispose lifecycle (`Service lifecycle`).
